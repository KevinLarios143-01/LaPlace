<template>
  <br>
  <br>
  <div class="home">
    <div class="container">
      <div class="card">
        <div class="row">
          <div class="col-md-12">
            <div class="card-header" style="text-align: center;">
              <h2 style="color: #62ce5c; font-family: monospace;">Detalles de la Reservación</h2>
            </div>
            <div class="card-body">
              <form class="row g-3">
                <form class="row g-3; was-validated">
                  <div class="col-md-5 offset-1">
                    <label for="nombre" style="font-weight:bold">Nombre de la persona:</label>
                    <input type="text" class="form-control" v-model="nombre"
                      placeholder="Ingrese Nombre de la Categoría" required>
                  </div>
                  <div class="col-md-5 offset-1">
                    <label for="nombre" style="font-weight:bold">Fecha:</label>
                    <input type="date" class="form-control" v-model="fecha" placeholder="Ingrese Nombre de la Categoría"
                      required>
                  </div>
                </form>
                <div class="col-md-4 offset-4">
                  <button type="button" class="btn btn-primary" align="center" @click="crear()">Buscar</button>
                </div>
              </form>
              <br>
              <div class="col-md-10 offset-1">
                <table class="table table-bordered table-sm table-hover mb-0 table table-striped">
                  <thead class="table-dark">
                    <tr>
                      <th style="text-align:center">Nombre</th>
                      <th style="text-align:center">Fecha</th>
                      <th style="text-align:center">Hora</th>
                      <th style="text-align:center">Cantidad de personas</th>
                      <th style="text-align:center">Total</th>
                    </tr>
                  </thead>
                  <tbody>
                    <template v-if="aCategoria.length > 0">
                      <tr v-for="(row, index) in aCategoria" :key="`row-${index}`">
                        <td style="text-align:center;" v-text="row.label"></td>
                        <td style="text-align:center;" v-text="row.code"></td>
                        <td style="text-align:center;" v-text="row.hora"></td>
                        <td style="text-align:center;" v-text="row.cantidad_personas"></td>
                        <td style="text-align:center;" v-text="row.total"></td>
                      </tr>
                    </template>
                    <template v-else>
                      <tr>
                        <td colspan="10">No hay filas para mostrar.</td>
                      </tr>
                    </template>
                  </tbody>
                </table>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import axios from 'axios';
export default {
  data() {
    return {
      id_categoria: "",
      nombre: "",
      precio: "",
      categoria: "",
      aCategoria: [],
      fecha: "",
      dia: "",
      mes: "",
      anio: ""
    }
  },
  mounted() {
    if (localStorage.getItem('vue3.isAuthenticated') != null) {
      this.$store.state.isAuthenticated = JSON.parse(localStorage.getItem('vue3.isAuthenticated'));
    }
    if (localStorage.getItem('vue3.isAdmin') != null) {
      this.$store.state.isAdmin = JSON.parse(localStorage.getItem('vue3.isAdmin'));
    }
  },
  methods: {
    crear() {
      axios({
        method: "get",
        url: "http://localhost:3000/reservacion/" + this.nombre + "/" + this.fecha,
        responseType: "json"
      }).then((response) => {
        this.aCategoria = [];
        if (response.status == 200) {
          for (let i = 0; i < response.data.categorias.length; i++) {
            this.fecha = new Date(response.data.categorias[i].fecha)
            this.dia = this.fecha.getUTCDate();
            this.mes = this.fecha.getUTCMonth() + 1;
            this.anio = this.fecha.getUTCFullYear();
            this.aCategoria.push({
              label: response.data.categorias[i].nombre,
              code: (this.dia + "-" + this.mes + "-" + this.anio),
              hora: response.data.categorias[i].hora,
              cantidad_personas: response.data.categorias[i].cantidad_personas,
              total: response.data.categorias[i].total
            });
          }
        }
      });
    },
  },
} 
</script>

<style>
.table-striped tbody tr:nth-of-type(odd) {
  background-color: #d9edf7 !important;
}

.table-hover tbody tr:hover {
  background-color: #f7f7f7 !important;
}
</style>